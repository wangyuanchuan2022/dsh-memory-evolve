...
/** Read the JSON request body (capped). Per-request memoized（写守卫前置
 * 读取后，各路由的 readBody 复用同一解析结果，流不被二次消费）。 */
async function readBody(req, maxBytes = 64 * 1024) {
  if (req.__dshApiBodyParsed) return req.__dshApiBody
  const chunks = []
  let total = 0
  ...
  if (chunks.length === 0) {
    req.__dshApiBodyParsed = true
    req.__dshApiBody = {}
    return {}
  }
  try {
    req.__dshApiBodyParsed = true
    req.__dshApiBody = JSON.parse(Buffer.concat(chunks).toString('utf8'))
    return req.__dshApiBody
  } catch {
...
export function installApi(ctx, deps) {
  ...
  const handler = async (req, res) => {
    ...
    try {
      // 统一写守卫（审计批次 2，D3-P1-2）：全部 POST/PUT 写端点前置校验
      // Content-Type=application/json + Origin 同源——此前 only /api/update
      // 受保护，其余约 20 个写端点可被跨站 no-cors 的 text/plain JSON 体
      // 驱动（fire-and-forget CSRF：改配置/删记忆/采纳技能/翻转同步开关）。
      // DELETE 豁免：非简单方法必然触发 CORS 预检，跨站表单/脚本不可达。
      // body 前置读取经 readBody 备忘录复用（各路由零改动）。
      if ((req.method === 'POST' || req.method === 'PUT') && path.startsWith('/memory-evolve/')) {
        let body
        try {
        ...
        if (guard) {
          sendJson(res, 400, { ok: false, code: 'cross-origin', error: guard })
          return
        }
      }
