...
export function promoteArchived(store, todoStore, archive, target, match, cwd) {
  ...
  // 审计 P1-1：writeTarget 校验前置——旧版本归档条目可能无原轨标记
  //（writeTarget 落到字面 todo-archive）、或原轨非法；todo-project 原轨
  // 还需要 cwd。此处友好报错而非让底层异常裸抛 400。
  if (writeTarget.startsWith('todo-')) {
    const track = writeTarget.slice(5)
    if (!['life', 'work', 'project', 'daily'].includes(track)) {
      return { ok: false, message: `归档条目缺少有效的原轨标记（${writeTarget}）——请手动处理该条目` }
    }
    if (track === 'project' && !cwd) {
      return { ok: false, message: '无法定位会话工作目录——project 待办转正需要在项目会话中操作' }
    }
  }
  const content = raw
  ...
  // 转正三步无跨步骤锁：add 与 remove 之间归档可能被并发修改（多义/
  // noMatch）——remove 失败时不再报裸成功，按 api.js:698 同款口径提示
  // 半完成态（归档页可手动清理；重复 ID 会在同步合并的 duplicates 报告
  // 中可见，审计 D3-P2-4）。
  const removal = archive.remove(target, match, cwd)
  if (removal.ok !== true) {
    return {
      ok: true,
      partial: true,
      message: `${rct('misc.promoted', { target: writeTarget, chars: content.length })}——但归档条目移除失败（${removal.message ?? '原因未知'}）：归档页可能出现重复条目，可手动清理`,
    }
  }
