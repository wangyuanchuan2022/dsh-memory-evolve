...
import { DEFAULT_DECAY_THRESHOLDS, generateDecayReport, normalizeDecayThresholds, readDecayCandidateCount } from './decay.js'
 ...
 */
function renderGlobalTrackInject(entries, config, headKey, summaryHeadKey) {
...
export function renderSnapshot(config, store, agent, counter, sessionTitleService = null, writeGap = null) {
  ...
  if (memoryEntries.length > 0) {
    ...
    parts.push(renderGlobalTrackInject(memoryEntries, config, 'snap.memoryHead', 'snap.memorySummaryHead'))
  }
  if (userEntries.length > 0) {
    parts.push(renderGlobalTrackInject(userEntries, config, 'snap.userHead', 'snap.userSummaryHead'))
  }
