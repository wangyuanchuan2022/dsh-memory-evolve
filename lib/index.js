...
import { ArchiveStore, MemoryStore, SuggestionQueue, extractEntryDate, gitBranch, gitBranchList, parseEntryBranches, parseEntryDshOnly, parseEntrySalience, parseEntrySummary, autoSummary, stripEntrySalience, stripEntrySummary, todayStamp } from './store.js'
import { stripEntryId, extractEntryId, legacyIdFor } from './sync/entryid.js'
import { bumpHits, hitKey, readSidecar, resolveUsedRefs } from './hit-stats.js'
import { DEFAULT_DECAY_THRESHOLDS, daysBetween, generateDecayReport, normalizeDecayThresholds, readDecayCandidateCount } from './decay.js'
...
export const DEFAULTS = {
  ...
  // memory/user 轨渐进式披露（autopilot-memory-lifecycle Step 4）：与 key 轨
  // 同机制的快照分层注入。off（默认）=始终全量，输出与旧版逐字节一致
  //（golden 基线守住）；auto=条目数与总字符均未超阈值才全量，超则摘要注入；
  // on=恒摘要。[salience:3] 条目在摘要模式下仍全文注入（重要性豁免）；
  // 摘要注入后取全文走 memory 工具 list（key 轨的 expand 是 key-only）。
  memoryProgressiveDisclosure: 'off',
  memoryFullInjectThreshold: 3,  // auto 模式：条目数 ≤ 此值 → 全量注入
  memoryFullInjectCharLimit: 1500, // auto 模式：总字符数 ≤ 此值 → 全量注入
  // 命中驱动的存量记忆豁免（PR #66 后续增强，块 5；用户需求：摘要模式下
  // 未标记 salience 的旧条目不应一刀切降为一行摘要——近期被实际使用过的
  // 旧记忆保持全文注入）。摘要分支的全文条件扩为 salience:3 **或**
  // 「hitCount≥1 且 lastAccessed 距今 ≤ 此天数」（daysBetween 同源口径）。
  // 0 = 关闭豁免（有命中也摘要）。
  memoryHitExemptDays: 30,
  // 衰减归档候选阈值（autopilot-memory-lifecycle Step 5，Q4 缺省决策）：
  // 数组按 salience 1/2/3 索引（天数），单项 0 = 该档永不进候选。初值
  // 30/90/180 无校准依据，上线后按首个真实 decay 报告的分布回看调整
  //（guji「抓完分布后校准」同款纪律）；改配置即可，无代码回退成本。
  decayThresholds: [30, 90, 180],
  // snapshot injection
  injectMemory: true,
  snapshotOrder: 500,
  injectionScan: true,
...
export const RUNTIME_KEYS = [
  ...
  'memoryProgressiveDisclosure', 'memoryFullInjectThreshold', 'memoryFullInjectCharLimit',
  'memoryHitExemptDays',
  'decayThresholds',
...
export function validateRuntimePatch(key, value) {
  switch (key) {
    ...
    case 'advisorMaxQueued':
      // 0 会让 runtime 的 queue.length >= maxQueued 恒真 → 评审请求全部
      // 静默 drop（静态校验 :521-523 一向拒绝 0——运行时对齐，审计 P1）
      if (typeof value !== 'number' || !Number.isInteger(value) || value < 1) {
        throw new Error('dsh-memory-evolve: advisorMaxQueued 必须是正整数（≥1）')
      }
      return
    ...
    case 'keyProgressiveDisclosure':
      ...
      return
    case 'memoryProgressiveDisclosure':
      if (value !== 'auto' && value !== 'off' && value !== 'on') {
        throw new Error('dsh-memory-evolve: memoryProgressiveDisclosure 必须是 "auto" / "off" / "on"')
      }
      return
    case 'keyFullInjectThreshold':
    case 'keyFullInjectCharLimit':
    case 'memoryFullInjectThreshold':
    case 'memoryFullInjectCharLimit':
      if (typeof value !== 'number' || !Number.isInteger(value) || value < 1) {
        throw new Error(`dsh-memory-evolve: ${key} 必须是正整数`)
      }
      return
    case 'memoryHitExemptDays':
      // 命中豁免天数：非负整数（0 = 关闭豁免），照 advisorMaxMessages 口径
      if (typeof value !== 'number' || !Number.isInteger(value) || value < 0) {
        throw new Error('dsh-memory-evolve: memoryHitExemptDays 必须是非负整数（0 = 关闭命中豁免）')
      }
      return
    case 'decayThresholds':
      // 长度 3、非负整数（0=永不）——与 lib/decay.js 的 normalize 同一口径
      normalizeDecayThresholds(value)
      return
    default:
...
const POSITIVE_NUMBER_KEYS = [
  ...
  // 分层注入数值旋钮（审计 P1：静态配置写错原先静默降级，违反
  // 「misconfiguration fails at load」自述——与运行时 :413-420 口径对齐）
  'keyFullInjectThreshold', 'keyFullInjectCharLimit',
  'memoryFullInjectThreshold', 'memoryFullInjectCharLimit',
]
const BOOLEAN_KEYS = [
...
export function resolveConfig(raw) {
  ...
  if (config.memoryProgressiveDisclosure !== 'auto'
    && config.memoryProgressiveDisclosure !== 'off'
    && config.memoryProgressiveDisclosure !== 'on') {
    throw new Error('dsh-memory-evolve: memoryProgressiveDisclosure 必须是 "auto" / "off" / "on"')
  }
  // 审计 P1：key 轨同款旋钮缺静态校验（写错静默恒摘要）——与运行时
  // validateRuntimePatch :403-407 口径对齐，双轨旋钮不再双标。
  if (config.keyProgressiveDisclosure !== 'auto'
    && config.keyProgressiveDisclosure !== 'off'
    && config.keyProgressiveDisclosure !== 'on') {
    throw new Error('dsh-memory-evolve: keyProgressiveDisclosure 必须是 "auto" / "off" / "on"')
  }
  // memoryHitExemptDays：非负整数（0=显式关闭豁免），照运行时 :423-428 口径
  if (!Number.isInteger(config.memoryHitExemptDays) || config.memoryHitExemptDays < 0) {
    throw new Error('dsh-memory-evolve: memoryHitExemptDays 必须是非负整数（0 = 关闭命中豁免）')
  }
  // 衰减阈值校验（Step 5，Q4）：归一写回（normalize 非法值响亮抛错）。
  if (config.decayThresholds !== undefined) {
    config.decayThresholds = normalizeDecayThresholds(config.decayThresholds)
  }
  return config
}

/**
 * 构建命中豁免键集（PR #66 后续增强，块 5）：扫描该轨侧车，收集
 * 「hitCount≥1 且 lastAccessed 距今 ≤ memoryHitExemptDays 天」的 hitKey。
 * daysBetween 与 lib/decay.js 同源口径；readSidecar 自带失败隔离（冷启动/
 * 损坏 → 空表 → 空集）。任何条件下返回 null = 无豁免（行为与现状一致）：
 *   - memoryHitExemptDays ≤ 0（0=用户显式关闭豁免）；
 *   - 目录缺失/侧车为空/无合格条目。
 * @param {object} config - resolved config。
 * @param {string} track - 记忆轨（'memory' | 'user'）。
 * @param {string | undefined} dir - 该轨侧车目录（memory/user = 记忆根）。
 * @returns {Set<string> | null} 豁免 hitKey 集；null = 无豁免。
 */
function buildHitExemptKeys(config, track, dir) {
  const days = config.memoryHitExemptDays
  if (!dir || typeof days !== 'number' || !Number.isInteger(days) || days <= 0) return null
  let stats
  try {
  ...
  for (const key of Object.keys(stats)) {
    ...
    if ((Number(rec.hitCount) || 0) < 1) continue
    const diff = daysBetween(rec.lastAccessed, today)
    // lastAccessed 缺失/非法（null）→ 保守跳过（无豁免依据）
    if (diff === null || diff > days) continue
    set.add(key)
  }
...
/**
 * memory/user 轨快照注入的三态渲染（autopilot-memory-lifecycle Step 4）：
 *   - off（默认）：全量注入，输出与旧版逐字节一致（golden 基线守住）；
 *   - auto：条目数 ≤ memoryFullInjectThreshold 且总字符 ≤
 *     memoryFullInjectCharLimit（双阈值合取，与 key 轨先例 :607-609 同构）
 *     才全量，超则摘要注入；
 *   - on：恒摘要注入。
 * 摘要注入形态（与 key 轨摘要行同款 `- [短id] 摘要`）：
 *   - [salience:3] 条目恒全文注入（重要性豁免，AC-3.2）；
 *   - 近期命中条目全文豁免（块 5，用户需求：存量旧记忆不被一刀切摘要）——
 *     hitCount≥1 且 lastAccessed 距今 ≤ memoryHitExemptDays 天，豁免集由
 *     调用方经 options.exemptKeys 传入（保持本函数纯度可测）；null/缺省
 *     = 无豁免（行为同现状，向后兼容旧调用）；
 *   - 其余条目一行摘要：显式 [summary] 优先、autoSummary 兜底；
 *   - 取全文走 memory 工具 list（key 轨的 expand 是 key-only，AC-3.5
 *     指引由摘要头文案承载）。
 * 与 key 轨的轨间差异：memory/user 无分支过滤/无 branch 变体头；
 * key 轨取全文走 expand、本函数走 list。
 * @param {string[]} entries - 该轨条目（原文）。
 * @param {object} config - resolved config。
 * @param {string} headKey - 全量注入头的 i18n key（snap.memoryHead / snap.userHead）。
 * @param {string} summaryHeadKey - 摘要注入头的 i18n key（含取全文指引）。
 * @param {object} [options] - { track?: string, exemptKeys?: Set<string> | null }。
 * @returns {string} 该轨的快照段文本。
 */
function renderGlobalTrackInject(entries, config, headKey, summaryHeadKey, options = {}) {
  const mode = config.memoryProgressiveDisclosure ?? 'off'
  const totalChars = entries.reduce((sum, e) => sum + e.length, 0)
  const useFullInject = mode === 'off' ||
    (mode === 'auto' && entries.length <= (config.memoryFullInjectThreshold ?? 3) && totalChars <= (config.memoryFullInjectCharLimit ?? 1500))
  if (useFullInject) {
    // 全量注入（现状逻辑）：展示剥身份证 + 摘要标记 + 重要性标记
    // （Step 3 的组合剥离链；对无 tag 条目零变化）
    return `${st(headKey)}\n${entries.map((entry) => `- ${stripEntrySalience(stripEntrySummary(stripEntryId(entry)))}`).join('\n')}`
  }
  // 摘要注入（渐进式披露）：保持原条目顺序逐条渲染——[salience:3] 全文占原位，
  // 近期命中条目全文豁免（块 5），其余一行摘要；确定性输出（同输入同输出，不排序不打乱）
  const { track = '', exemptKeys = null } = options
  const lines = entries.map((entry) => {
    if (parseEntrySalience(entry) === 3) {
      return `- ${stripEntrySalience(stripEntrySummary(stripEntryId(entry)))}`
    }
    if (exemptKeys !== null && exemptKeys !== undefined && track !== '' && exemptKeys.has(hitKey(track, entry))) {
      return `- ${stripEntrySalience(stripEntrySummary(stripEntryId(entry)))}`
    }
...
export function renderSnapshot(config, store, agent, counter, sessionTitleService = null, writeGap = null) {
  ...
  // 块 5：命中豁免集只在分层注入启用时构建（off=纯全量路径零侧车读取）；
  // memory/user 轨侧车同在记忆根（键前缀区分轨道），每轨一次 readSidecar。
  const disclosureActive = (config.memoryProgressiveDisclosure ?? 'off') !== 'off'
  const memoryExempt = memoryEntries.length > 0 && disclosureActive
    ? buildHitExemptKeys(config, 'memory', config.memoryDir) : null
  const userExempt = userEntries.length > 0 && disclosureActive
    ? buildHitExemptKeys(config, 'user', config.memoryDir) : null
  if (memoryEntries.length > 0) {
    // 展示剥离身份证（Codex 二轮 P1-4）：全局轨启用后补发的 [id:xxxx]
    // 不得进入模型上下文（与 KEY 段同规则）。Step 3：salience tag 同剥；
    // Step 4：分层注入三态（off=全量现状逐字节同 golden 基线 / auto / on）
    parts.push(renderGlobalTrackInject(memoryEntries, config, 'snap.memoryHead', 'snap.memorySummaryHead', { track: 'memory', exemptKeys: memoryExempt }))
  }
  if (userEntries.length > 0) {
    parts.push(renderGlobalTrackInject(userEntries, config, 'snap.userHead', 'snap.userSummaryHead', { track: 'user', exemptKeys: userExempt }))
  }
  ...
  if (keyEntries.length > 0) {
    ...
    if (useFullInject) {
      ...
      // Step 3：salience tag 同剥（对无 tag 条目零变化）
      parts.push(`${head}\n${keyEntries.map((entry) => `- ${stripEntrySalience(stripEntrySummary(stripEntryId(entry)))}`).join('\n')}`)
    } else {
      // 摘要注入（渐进式披露）
      const head = branch !== undefined
...
${st('snap.readHint')}${branchHint}${todoHint}`)
  ...
  if (writeTargets.length > 0 || keyDuty || reviewOn) {
    ...
    const dueWarning = due
      ...
      : ''
    // 审查联动衰减候选（Step 5，AC-5.1~5.3；Q8 一期合并计数）：due 且
    // decay-report.json 存在且非空 → dueWarning 文案附加候选数，提示模型
    // 本轮审查可一并消费该报告。只读报告文件（AC-5.3，readDecayCandidate-
    // Count 无任何写路径）；报告缺失/为空/损坏 → 不附加（快照与无联动版
    // 逐字节一致——AC-5.2 负向，tests/snapshot-golden.test.js golden +
    // tests/review-decay-link.test.js 双重钉住）。候选数变化属低频文本
    // 变化，与 dueWarning 同款缓存代价。
    const decayHint = due && reviewOn
      ? (() => {
          // P2-2 修复：兜底口径与 DEFAULTS 对齐——memoryDir 缺失时跳过读取
          //（候选数按 0），不再以 '' 拼出相对路径读到进程 cwd 的同名文件。
          const n = config.memoryDir ? readDecayCandidateCount(config.memoryDir) : 0
          return n > 0 ? st('snap.dueDecayHint', { count: n }) : ''
        })()
      : ''
...
${tail}${dueWarning}${decayHint}${writeWarning}`)
  }
...
function outcomeOnly(result) {
  if (result && typeof result === 'object') {
    // duplicate（store.add 去重路径的内部簿记标记）与 entries/removed 一样
    // 不在 output schema 声明里——带着返回会被宿主 additionalProperties:false
    // 整单拒绝（2026-09-24 审计 P0，同 retag entry 1d2534f 型）。
    const { entries, removed, matches, duplicate, ...rest } = result
...
export function memoryTool(ctx, config, store, queue, getRuntime, archive, writeGap = null) {
   ...
   * @param {string|undefined} summary - summary 参数（仅 key 轨生效）。
   * @param {number|undefined} salience - salience 参数（1-3；仅 memory/user/
   *   key 轨写 tag，project/daily 忽略；已在工具层钳制/拒绝）。
   * @returns {Promise<object>} 工具友好结果 {ok, message, target, ...}。
   */
  async function addOne(target, content, feedback, exec, summary, salience) {
    ...
    // 重要性 tag（Step 3，AC-2.1）：key 轨在 addOne 预拼进 finalContent
    //（随建议队列落盘；stampEntry 侧有防重，不会双 tag），位置 =
    // [summary:…]（若有）之后、正文之前。memory/user 走 store.add 的
    // salience 参数由 stampEntry 统一插入；project/daily 日志轨不标（忽略）。
    if (target === 'key' && salience !== undefined && salience !== null) {
      const n = Number(salience)
      if (Number.isInteger(n) && n >= 1 && n <= 3) {
        const sumMatch = /^\[summary:[^\]]*\]\s*/.exec(finalContent)
        const tag = `[salience:${n}] `
        finalContent = sumMatch !== null
          ? finalContent.slice(0, sumMatch[0].length) + tag + finalContent.slice(sumMatch[0].length)
          : tag + finalContent
      }
    }
    if (target === 'key') {
    ...
    const addResult = store.add(target, finalContent, exec.agent, salience)
  ...
  return {
    ...
    parameters: {
      ...
      properties: {
        action: {
          type: 'string',
          enum: ['add', 'replace', 'remove', 'archive', 'list', 'expand', 'decay', 'retag'],
        ...
        salience: {
          type: 'integer',
          get description() { return mt('param.salience') },
        },
        used: {
          type: 'array',
          items: { type: 'string' },
          get description() { return mt('param.used') },
        },
      },
        ...
        switch (action) {
          case 'list': {
            ...
            if (args.archived === true) {
              ...
              result = {
                ...
                target,
                // 展示剥离身份证+摘要标记+重要性标记（§4.7；Step 3 补 salience）
                entries: entries.map((entry) => stripEntrySalience(stripEntrySummary(stripEntryId(entry)))),
                chars: entries.map((entry) => stripEntrySalience(stripEntrySummary(stripEntryId(entry)))).join('\n').length,
              }
              break
            }
            const stats = {}
            ...
            result = {
              ...
              // （施工图 §4.7；内部匹配用原文，不受影响）。Step 3：salience
              // tag 同为程序元数据，工具回显一并剥离（AC-2.4）。
              entries: entries.map((entry) => stripEntrySalience(stripEntrySummary(stripEntryId(entry)))),
              chars: entries.map((entry) => stripEntrySalience(stripEntrySummary(stripEntryId(entry)))).join('\n').length,
              total,
              earliest,
          ...
          case 'add': {
            ...
            if (Array.isArray(args.entries) && args.entries.length > 0) {
              ...
              // 块 7：批量写主流程完成后处理 used 使用申报（主流程优先）
              applyUsedReporting(result, args, store, exec)
              break
            }
            // ── 单轨 add（原逻辑 + feedback 参数）──
            let content = String(args.content ?? '').trim()
            let branchWarning = ''
            // 重要性归一（Step 3，AC-2.5；修复轮 P1-3 抽公用 helper
            // normalizeSalienceFor，与 replace 同链——见该函数 JSDoc）。
            const norm = normalizeSalienceFor(target, args.salience)
            if (norm.reject) {
              result = { ...norm.reject, target }
              break
            }
            let salience = norm.salience
            let salienceNote = norm.note
            ...
            const addOneResult = await addOne(target, content, args.feedback, exec, args.summary, salience)
            if (addOneResult.ok && branchWarning !== '') addOneResult.message += branchWarning
            if (addOneResult.ok && salienceNote !== '') addOneResult.message += salienceNote
            result = addOneResult
            // 块 7：单轨写主流程完成后处理 used 使用申报（主流程优先）
            applyUsedReporting(result, args, store, exec)
            break
          }
          case 'replace': {
            // 修复轮 F-1（安全评审 LOW-1）：replace 与 add 同一归一链——
            // 此前 salience 在此被静默丢弃，模型以为标了重要性、条目实际
            // 按无 tag（中档）参与衰减调度；现在钳制/拒绝/日志轨忽略语义
            // 与 add 完全一致。
            const norm = normalizeSalienceFor(target, args.salience)
            if (norm.reject) {
              result = { ...norm.reject, target }
            ...
            result = outcomeOnly(store.replace(target, args.match, args.content, exec.agent, norm.salience))
            if (result.ok && norm.note !== '') result.message += norm.note
            break
          }
          case 'retag': {
            // 存量补标（块 6，用户拍板：主路径=运行时由大模型统一为旧记忆
            // 补标）：按 match 定位单条，剥旧 [salience:N] 盖新级别——正文
            // 逐字不动、[id:]/[summary:] 原样保留；支持矩阵与 add 相同
            //（memory/user/key 有效，project/daily 诚实拒绝——retag 的唯一
            // 目的就是打 tag，静默忽略会让模型误以为已生效）。
            if (target === 'project' || target === 'daily') {
              result = { ok: false, message: mt('msg.retagLogsRejected', { track: target }), target }
              break
            }
            const norm = normalizeSalienceFor(target, args.salience)
            if (norm.reject) {
            ...
            if (norm.salience === undefined) {
              result = { ok: false, message: mt('msg.retagNeedsSalience'), target }
              break
            }
            result = outcomeOnly(store.retag(target, args.match, norm.salience, exec.agent))
            if (result.ok) {
              // 回显含短 id 与新级别确认（规格块 6.3）
              const shortId = extractEntryId(result.entry) ?? legacyIdFor(result.entry)
              result.message = mt('msg.retagDone', { shortId, level: norm.salience })
              if (norm.note !== '') result.message += norm.note
              // store 的 entry 字段仅本块内取短 id 用；output schema 是
              // additionalProperties:false，带着返回会被宿主校验整单拒绝
              //（2026-09-24 生产实证——假宿主测试不校验输出，故静态契约锁
              // 在 tests/retag.test.js ⑦）。
              delete result.entry
            }
            break
          }
          ...
          case 'expand': {
            ...
            result = {
              ...
              target,
              entries: [stripEntrySalience(stripEntrySummary(stripEntryId(found)))],
            }
            break
          }
          case 'decay': {
            // 衰减归档候选报告（Step 5，spec 候选 3 / AC-4 全组）：扫描
            // memory/user/key 三轨产候选清单，**只读派生**——绝不删除、
            // 绝不归档任何条目（非目标 5），唯一写动作 = 覆盖写
            // <memoryDir>/decay-report.json。target 参数对本操作无语义
            //（schema required 仍要求 target，传任意合法轨即可，本分支忽略）。
            try {
              const { report, count } = generateDecayReport(
                store,
                exec?.agent,
                config.decayThresholds ?? DEFAULT_DECAY_THRESHOLDS,
              )
              result = {
                ok: true,
                message: count > 0 ? mt('msg.decayDone', { count }) : mt('msg.decayNone'),
                target,
                // 候选数随回显带出（零候选 = 0，AC-4.6 明示）
                total: count,
                latest: report.metadata.generatedAt,
              }
            } catch (error) {
              result = { ok: false, message: mt('msg.decayFailed', { detail: error?.message ?? String(error) }), target }
            }
            break
          }
          default:
...
/**
 * used 使用申报处理（块 7，PR #66 后续增强；用户洞察：模型实际只从注入
 * 快照读记忆、几乎不调 list/expand，命中信号必须能从「使用申报」流入）：
 * 主流程写入成功后，解析 args.used 申报的独特子串并为命中条目 bumpHits。
 * 解析顺序 = 先写 entries（主流程优先）、后处理 used；全程 try/catch 失败
 * 隔离——used 解析任何异常绝不影响 add 主流程（返回值与 message 主干）。
 * 命中时在 result.message 追加「已记录 N 条记忆引用（M 条未匹配已忽略）」
 * （N=0 时整段省略不显）。
 * @param {object} result - add 主流程的结果对象（原地追加回显）。
 * @param {object} args - 工具调用参数（读 args.used）。
 * @param {import('./store.js').MemoryStore} store - memory store。
 * @param {object} exec - 工具执行上下文（取 agent 定位 key 轨目录）。
 * @returns {object} 同一 result 对象。
 */
function applyUsedReporting(result, args, store, exec) {
  try {
    const refs = args?.used
    if (!result?.ok || !Array.isArray(refs) || refs.length === 0) return result
    const agent = exec?.agent
    const trackEntries = {}
    const trackDirs = {}
    for (const track of ['memory', 'user', 'key']) {
      try {
        const dir = store.locate(track, agent)?.dir
        if (!dir) continue
        trackDirs[track] = dir
        trackEntries[track] = store.entriesOf(track, agent)
      } catch {
        // 单轨读取失败 → 该轨跳过（失败隔离，其余轨继续）
      }
    }
    const { hits, unmatched } = resolveUsedRefs(trackEntries, refs)
    for (const hit of hits) bumpHits(trackDirs[hit.track], hit.track, [hit.entry])
    if (hits.length > 0) {
      result.message += ` ${mt('msg.usedRecorded', { count: hits.length, unmatched: unmatched.length })}`
    } else if (unmatched.length > 0) {
      // N=0：不显「已记录」段，但保留诚实忽略回显（M>0 时）
      result.message += ` ${mt('msg.usedIgnored', { unmatched: unmatched.length })}`
    }
  } catch (error) {
    // 块 7 失败隔离契约：used 任何异常不影响 add 主流程结果
    console.error(`[used] 使用申报解析失败（已忽略）: ${error && error.message ? error.message : error}`)
  }
  return result
}

/**
 * 单轨 add / replace 共用的 salience 参数归一（Step 3，AC-2.5；修复轮
 * P1-3 + 安全评审 F-1 抽公用，两动作同链同语义）。
 *   - undefined/null → 不标 tag、无附加回显；
 *   - Number() 后非整数 → reject（写入整体拒绝，不落任何条目）；
 *   - project/daily 日志轨 → 该轨本就不写 tag（stampEntry 口径），归一为
 *     undefined 并回显「已忽略」——修复前对越界值回显「已钳制」但实际
 *     什么都不写，属误导性成功回显（P1-3）；
 *   - memory/user/key → 越界整数钳制 [1,3] 并回显「已钳制」，合法值原样。
 * @param {string} target - 记忆轨。
 * @param {number|string|undefined|null} raw - 工具层收到的 salience 原始值。
 * @returns {{salience: (number|undefined), note: string, reject?: object}}
 */
function normalizeSalienceFor(target, raw) {
  if (raw === undefined || raw === null) return { salience: undefined, note: '' }
  const n = Number(raw)
  if (!Number.isInteger(n)) {
    return {
      salience: undefined,
      note: '',
      reject: { ok: false, message: mt('msg.salienceRejected', { value: String(raw) }) },
    }
  }
  if (target === 'project' || target === 'daily') {
    return { salience: undefined, note: ` ${mt('msg.salienceIgnoredTrack', { track: target })}` }
  }
  const clamped = Math.min(3, Math.max(1, n))
  if (clamped !== n) {
    return { salience: clamped, note: ` ${mt('msg.salienceClamped', { from: n, to: clamped })}` }
  }
  return { salience: n, note: '' }
}
...
export function buildMemoryContext(store, { cwd, branch: declaredBranch, tracks, excludeDshOnly } = {}) {
  ...
  if (want('memory')) {
    ...
    if (memoryEntries.length > 0) {
      // 修复轮 P2-1：memory/user 段同剥 salience——AC-2.4「全量注入不含
      // tag」的覆盖延伸到本通道；[id:]/[summary:] 的原文注入为既有设计
      // （评审 P2-1 选项①的窄口径：只剥本批新引入的 salience tag）。
      parts.push(`${st('ctx.memoryGlobal')}\n${memoryEntries.map((e) => stripEntrySalience(e)).join('\n')}`)
    }
  }
  if (want('user')) {
    const userEntries = dshOnlySafe(store.entriesOf('user'))
    if (userEntries.length > 0) {
      parts.push(`${st('ctx.userProfile')}\n${userEntries.map((e) => stripEntrySalience(e)).join('\n')}`)
    }
  }
  if (want('key') && cwd) {
    const keyAgent = { session: { header: { cwd } } }
    ...
    if (keyEntries.length > 0) {
      ...
      // [summary:…] 是仅供摘要模式用的元数据，不注入外部执行器）；Step 3：
      // salience tag 同剥（AC-2.4「全量注入不含 tag」涵盖本注入通道）
      parts.push(`${head}\n${keyEntries.map((entry) => stripEntrySalience(stripEntrySummary(stripEntryId(entry)))).join('\n')}`)
    }
  }
  return parts.join('\n\n')
