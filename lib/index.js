...
export function renderSnapshot(config, store, agent, counter, sessionTitleService = null, writeGap = null) {
  ...
  if (memoryEntries.length > 0) {
    ...
    // 不得进入模型上下文（与 KEY 段同规则）。Step 3：salience tag 同为
    // 程序元数据，全量注入一并剥离（对无 tag 条目零变化——golden 基线守）。
    parts.push(`${st('snap.memoryHead')}\n${memoryEntries.map((entry) => `- ${stripEntrySalience(stripEntrySummary(stripEntryId(entry)))}`).join('\n')}`)
  }
  if (userEntries.length > 0) {
    parts.push(`${st('snap.userHead')}\n${userEntries.map((entry) => `- ${stripEntrySalience(stripEntrySummary(stripEntryId(entry)))}`).join('\n')}`)
  }
