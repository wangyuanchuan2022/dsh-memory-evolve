...
${st('snap.readHint')}${branchHint}${todoHint}`)
  ...
  if (writeTargets.length > 0 || keyDuty || reviewOn) {
    ...
    const decayHint = due && reviewOn
      ? (() => {
          const n = readDecayCandidateCount(config.memoryDir ?? '')
      ...
      try {
        switch (action) {
          ...
          case 'add': {
            ...
            let branchWarning = ''
            // 重要性归一（Step 3，AC-2.5）：undefined/null 跳过；非整数
            // 拒绝并回显（不落任何条目）；越界整数钳制 [1,3] 并回显。
            // 所有轨统一校验（输入合法性不随轨变化）；写入侧只对
            // memory/user/key 生效（project/daily 日志轨不标，静默忽略）。
            let salience = args.salience
            let salienceNote = ''
            if (salience !== undefined && salience !== null) {
              const n = Number(salience)
              if (!Number.isInteger(n)) {
                result = { ok: false, message: mt('msg.salienceRejected', { value: String(salience) }), target }
                break
              }
              const clamped = Math.min(3, Math.max(1, n))
              if (clamped !== n) {
                salience = clamped
                salienceNote = ` ${mt('msg.salienceClamped', { from: n, to: clamped })}`
              }
            } else {
              salience = undefined
            }
