 ...
 * [summary:…]——与 parseEntrySalience 同一头部序列），正文中出现的
 * 字面 [salience:…] 不动。对无 tag 条目零变化（黄金基线守住）。
 * 与 stripEntrySummary 组合使用（stripEntrySalience(stripEntrySummary(…))），
 * 两函数各自幂等、任意组合等价于按序剥离。
 * @param {string} entry - 完整条目文本。
 * @returns {string} 无 salience 标记的条目文本。
 */
export function stripEntrySalience(entry) {
  const match = ENTRY_HEAD_RE.exec(String(entry ?? ''))
  const prefix = match === null ? '' : match[0]
  let rest = String(entry ?? '').slice(prefix.length)
  // 与 parseEntrySalience 同一头部序列（可选 summary → 可选 salience，容错乱序）
  const salFirst = SALIENCE_HEAD_TOKEN_RE.exec(rest)
  if (salFirst !== null) {
    rest = rest.slice(salFirst[0].length)
    const sumSecond = SUMMARY_HEAD_TOKEN_RE.exec(rest)
    if (sumSecond !== null) rest = rest.slice(sumSecond[0].length)
  } else {
    const sumFirst = SUMMARY_HEAD_TOKEN_RE.exec(rest)
    if (sumFirst !== null) {
      rest = rest.slice(sumFirst[0].length)
      const salSecond = SALIENCE_HEAD_TOKEN_RE.exec(rest)
      if (salSecond !== null) rest = rest.slice(salSecond[0].length)
    }
  }
  return prefix + rest
}
