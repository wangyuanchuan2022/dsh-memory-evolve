...
export const MEMORY_DICT = {
  ...
  'param.used': [
    'add 可选（使用申报）：每轮收尾写入时，如本轮从注入内容中实际参考/用到了既有记忆，把那些条目的独特子串（条目标题或开头短语）放进 used——服务端按 memory→user→key 轨序解析，某轨恰一条命中即为该条目命中计数 +1（用于重要性调度与衰减判定）；未匹配/多义的 ref 忽略不报错',
    'Optional for add (usage report): at end-of-turn writes, if this turn actually consulted existing memories from the injected context, put their distinctive substrings (entry title or opening phrase) into used — the server resolves each ref across memory→user→key, and a unique hit on any track bumps that entry\'s hit count (feeding importance scheduling and decay screening); unmatched/ambiguous refs are ignored without error',
  ],
  ...
  // 存量补标（块 6，retag action）回显与守卫
  'msg.retagDone': [
    '已重打记忆重要性：[短id {shortId}] → salience {level}（正文逐字未动）',
    'Retagged: [id {shortId}] → salience {level} (body text unchanged)',
  ],
  'msg.retagNeedsSalience': [
    'retag 必须提供 salience（整数 1-3：3=核心/架构，2=重要，1=低）',
    'retag requires a salience value (integer 1-3: 3=core/architecture, 2=important, 1=low)',
  ],
  'msg.retagLogsRejected': [
    '{track} 是追加型日志轨，不支持 retag 重打重要性（日志条目无重要性语义）',
    'the {track} track is an append-only log and does not support retag (log entries carry no importance)',
  ],
  // used 使用申报（块 7）回显：N=命中数、M=未匹配数（N=0 时省略已记录段，
  // 仅在 M>0 时显诚实忽略回显）
  'msg.usedRecorded': [
    '已记录 {count} 条记忆引用（{unmatched} 条未匹配已忽略）',
    'Recorded {count} memory usage ref(s) ({unmatched} unmatched, ignored)',
  ],
  'msg.usedIgnored': [
    '（{unmatched} 条使用申报未匹配，已忽略）',
    '({unmatched} usage ref(s) unmatched, ignored)',
  ],
  'msg.salienceClamped': [
...
export const SNAPSHOT_DICT = {
  ...
  'snap.batchWriteDuty': [
    '用 memory 工具**一次调用**（action=add + entries 数组，含 {targets} 各一项）写 1 条本回合进展（1-2 行具体内容；本轮实际用到的既有记忆，请在同次调用的 used 参数申报其独特子串，命中计数用于重要性调度与衰减判定）',
    'In ONE memory call (action=add with an entries array containing one item each for {targets}) write one entry of this turn\'s progress (1-2 concrete lines; for existing memories this turn actually used, report their distinctive substrings via the same call\'s used parameter — hit counts feed importance scheduling and decay screening)',
  ],
  'snap.and': [' 与 ', ' and '],
  'snap.keyDuty': [
    '本轮出现重要项目事实（长期约定/决策/架构/踩坑）时另向 target=key 提交 1 条建议（用户确认后写入并注入；核心约定/决策可传 salience:2-3 标注重要性，常规进展不传）；存量旧条目缺重要性标记时用 retag 补标（3=核心约定/架构/高频复用事实，2=重要，1 或不标=常规进展），建议在记忆审查到期轮分批处理（每轮 10-20 条至清零）；没有则跳过',
    'when durable project facts appear this turn (long-lived conventions/decisions/architecture/pitfalls), additionally submit one suggestion to target=key (written and injected after user confirmation; pass salience:2-3 for core conventions/decisions, omit for routine progress); retag legacy entries that lack an importance mark (3=core conventions/architecture/high-reuse facts, 2=important, 1 or unset=routine progress), ideally in batches during memory-review turns (10-20 entries per turn until done); skip when there are none',
  ],
  'snap.feedbackDuty': [
...
export const STORE_DICT = {
  ...
  'store.retagged': ['已重打重要性（{target} 轨 → salience {level}）', 'Retagged ({target} track → salience {level})'],
  'store.retagBadLevel': [
    'salience 必须是 1-3 的整数（收到 "{value}"）',
    'salience must be an integer between 1 and 3 (got "{value}")',
  ],
  'store.driftGuardOp': [
    '拒绝操作：{file} 的内容无法通过记忆工具解析往返。已备份到 {backup}。请先整理文件再重试。',
    'Operation refused: {file} does not round-trip through the memory tool parser. A backup was saved to {backup}. Reformat the file first, then retry.',
