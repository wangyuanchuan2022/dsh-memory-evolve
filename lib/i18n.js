...
export const MEMORY_DICT = {
  ...
  'param.salience': [
    'add 可选：条目重要性 1-3（1=低 2=中 3=高，整数；仅 memory/user/key 轨写标记，project/daily 不标；缺省=中档且不写标记；越界自动钳制到 1-3）',
    'Optional for add: entry importance 1-3 (1=low, 2=medium, 3=high, integer; written for memory/user/key tracks only, not for project/daily; omit for medium with no tag; out-of-range values are clamped into 1-3)',
  ],
...
export const SNAPSHOT_DICT = {
  ...
  'snap.memorySummaryHead': [
    '## 长期记忆（所有项目、会话都必须遵循；摘要模式，仅注入摘要——需要某条全文时用 memory 工具 list（target=memory）获取）',
    '## Long-term memory (every project and session must follow this; summary mode — only summaries are injected; use the memory tool with list (target=memory) to load any entry in full)',
  ],
  'snap.userSummaryHead': [
    '## 用户档案（摘要模式，需要某条全文时用 memory 工具 list（target=user）获取）',
    '## User profile (summary mode; use the memory tool with list (target=user) to load any entry in full)',
  ],
