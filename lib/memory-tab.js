...
import { parseEntries, parseEntrySalience, stripEntrySalience, stripEntrySummary } from './store.js'
  ...
  return rows.map((row) => {
    ...
    out.content = text
      ...
      .map((chunk) => {
        ...
        // [salience:N] 与 [summary:] 同为程序元数据：正文完整显示时一并剥掉
        //（重要性走 entryMeta 徽标展示，不重复显示原文 tag）。
        return trimmed === '' ? chunk : stripEntrySalience(stripEntrySummary(trimmed))
      })
      .join('\n§\n')
